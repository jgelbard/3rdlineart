...
$reviewer=mysql_query("SELECT * FROM reviewer", $bd); 
    while ($row_reviewer=mysql_fetch_array($reviewer)){
        ...
        $assigned_forms=mysql_query("SELECT * FROM assigned_app_results where rev_id=' $id'", $bd);
        $count = mysql_numrows ($assigned_forms);
        
        $assigned_forms_reviewed=mysql_query("SELECT * FROM assigned_app_results where status = 'Reviewed' and rev_id=' $id'", $bd);
