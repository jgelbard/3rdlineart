...
<?php
 function makebuttons($btname) {
    ...
    $buttons = [
        ...
        'man_supp' => ['Supply Chains', 'select_supp' ],
    ];
    foreach ($buttons as $name => $label_query) {
        ...
        // $num_forms = '?';
        echo '<li class="'.($btname==$name?'active':'').'"><a href="dash.php?'.$p.'"><i class="icon-th-list" style="font-size: 16px;"></i>&nbsp('.$num_forms.')<span style="font-size: 14px;">'.$label.'</span> </a> </li>';
    }
}
