...
$assigned_forms=mysql_query("SELECT distinct form_id,date_assigned FROM assigned_app_results WHERE form_id in (select form_id from reviewer_team_lead2 where reviewer_team_lead2.rev_id=$rev_id) and form_id not in (select form_id from expert_review_consolidate2) ORDER BY `assigned_app_results`.`form_id` DESC", $bd); 


$num_forms = mysql_num_rows ($assigned_forms);
...
echo '<p>Total forms: [ <i>'. $num_forms .'</i> ]</p>';
    ...
    $select_team_lead=mysql_query("SELECT * FROM reviewer_team_lead where form_id='$form_id'", $bd);
    ...
    while ($row_assigned=mysql_fetch_array($assigned)){
       ...
       $rev_status =$row_assigned['status']; 
            ...
            $select_reviewer=mysql_query("SELECT * FROM reviewer where id='$rev_id_as'", $bd); 
       ...
       ';
        ...
        echo '</strong></p> </td>';
         ...
         if ($complete_review >='3'){
               ...
               echo ' 
                <td class="td-actions"><a href="review_p1.php?consolidate_result&formid='.$form_id.'" class="btn btn-large btn-success" style="color:#fff"><i class="btn-icon-only icon-ok"> '. $rev_id.'Consolidate Reviews </i></a></td>
                  </tr> 
        
        ';
             /*echo ' 
                <td class="td-actions"><a href="cp_p1.php?consolidate&formid='.$form_id.'" class="btn btn-large btn-invert" style="color:#f00"><i class="btn-icon-only icon-ok"> '. $team_leader_id.'Notify Lead Expert </i></a></td>
                  </tr> 
